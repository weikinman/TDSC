/**
     ...
     * @example
var MrSoya = new TDSC.Text({
    text:"Hi~~,i'm MrSoya"
});
var path = new TDSC.Path("M250 150 L150 350 L350 350 Z");
var pt = new TDSC.PathTween(MrSoya,
        path,
        1000,{easing:TDSC.Tween.Bounce.Out}
    );
    * @param {Object} target 需要进行对象
    * @param {TDSC.Path} path 路径实例
    * @param {int} duration 补间周期(ms)
    * @param {Object} opts 补间属性
    * @param {function} [opts.easing=TDSC.Tween.Linear] 补间类型
    ...
    * @class
    * @see {TDSC.Tween.Linear}
    * @author {@link http://weibo.com/TDSC MrSoya}
    * @since 1.2.0
    */
TDSC.PathTween = function(target,path,duration,opts){
...
TDSC.PathTween.prototype = {
            ...
            switch(type){
                ...
                case 'l':
                    ...
                    for(var i=0;i<xys.length;i+=2){
                        ...
                        var len = TDSC.Math.len2D(sx,sy,xys[i],xys[i+1]);
                ...
                case 'c':
                    ...
                    for(var i=0;i<pps.length-2;i+=2){
                        var len = TDSC.Math.len2D(pps[i],pps[i+1],pps[i+2],pps[i+3]);
                ...
                case 'z':
                    ...
                    var len = TDSC.Math.len2D(sx,sy,ox,oy);
    ...
    start:function(){
        ...
        if(this.target.__pt instanceof TDSC.PathTween){
    ...
    update:function(now,d){
        ...
        if(this.onUpdate)this.onUpdate(t,ratio,this.__radian*TDSC.Math.ONEANG);
    },
    /**
     * 销毁补间实例，释放内存
     */
     ...
     * @param  {TDSC.PathTween} tween 下一个补间
     * @return this
     */
    next:function(tween){
        this.__next = tween;
        if(this.iteration===-1){
            TDSC.console.warn('invalid [next] setting on infinite loop instance...');
        }
        return this;
    }
};
...
/********* 扩展 **********/
TDSC.ext({},/** @lends TDSC.DisplayObject.prototype */{
    /**
    * 播放路径补间动画
    * @param {TDSC.Path} path 路径
    * @param {int} duration 补间周期(ms)
    * @param {Object} [opts] 补间属性
    * @param {function} [opts.easing=TDSC.Tween.Linear] 补间类型
    ...
    * @see {TDSC.Tween.Linear}
    * @return {TDSC.PathTween} 补间实例
    * @requires tween
    */
    pathAnimate:function(path,duration,opts,si){
        var tween = new TDSC.PathTween(this,path,duration,opts);
